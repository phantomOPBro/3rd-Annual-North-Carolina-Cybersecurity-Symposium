...
checkpoint = ModelCheckpoint('NCPACE_CNN_Train.h5', verbose=1, monitor='val_loss',
                             save_best_only=True,
                             mode='auto')
...
with open('NCPACE_CNN_Train_lossacc.txt', 'w') as file:
    file.write(f'Loss: {loss}\n')
    file.write(f'Accuracy: {accuracy}\n')

# Save the tokenizer
with open('NCPACE_CNN_Train_tokenizer.pickle', 'wb') as handle:
    pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Save the training history
history_df = pd.DataFrame(history.history)
history_df.to_csv('NCPACE_CNN_Train_history.csv')

# Save the model
model.save('NCPACE_CNN_Train_model.h5')
...
loaded_model = tf.keras.models.load_model('NCPACE_CNN_Train_model.h5')
tf.saved_model.save(loaded_model, 'saved_model')
!python -m tf2onnx.convert --saved-model 'saved_model' --output 'NCPACE_CNN_Train_model.onnx'
...
# Load the model
model = load_model('NCPACE_CNN_Train_model.h5')

# Load the tokenizer
with open('NCPACE_CNN_Train_tokenizer.pickle', 'rb') as handle:
    tokenizer = pickle.load(handle)

input_size = 100

# The filename to predict
filename = 'NCPACE_is_the_best_conference_ever.pdf'
...
# Load the ONNX model
sess = onnxruntime.InferenceSession("NCPACE_CNN_Train_model.onnx")
